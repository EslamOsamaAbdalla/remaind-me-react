...
function CreateTask({settaskItem}) {
    ...
    return (
        <>
        <button className='add-task-btn' onClick={showPage}>add task</button>
        <div className='add-task-page' ref={elem}>
            <div className='close' onClick={hidePage}>x</div>
            <form>
                <label>task</label>
                ...
                <button className='add-task-submist' onClick={taskData}>add</button>
            </form>
        </div>
        </>
    )
